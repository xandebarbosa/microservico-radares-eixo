...
CREATE TABLE radars_eixo (
    ...
    CONSTRAINT pk_radars_eixo PRIMARY KEY (id, data),
...
CREATE INDEX idx_radars_placa_gin ON radars_eixo USING GIN (placa gin_trgm_ops);

-- B. Índice Composto para Filtros (Cobre a busca por Local + Data + Hora)
-- Ordem estratégica: Praca (filtro macro) -> Km -> Data -> Hora
CREATE INDEX idx_radars_filtros_main ON radars_eixo (praca, km, data, hora);

-- C. Índice Cronológico
-- Para ordenar "os mais recentes primeiro" sem fazer "sort" em memória.
CREATE INDEX idx_radars_data_hora ON radars_eixo (data DESC, hora DESC);

-- D. Índice de Join
CREATE INDEX idx_radars_localizacao_id ON radars_eixo(localizacao_id);
